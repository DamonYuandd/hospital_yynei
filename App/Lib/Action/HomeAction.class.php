...
class HomeAction extends CommonAction
{
	...
	//	if( strpos($domain,$_SERVER["HTTP_HOST"])!==false ) {//设置手机域名
		...
		//獲取友情鏈接
		$orderStr = 'ordernum desc,id desc';
		$link1 = M('Link')->where(array('category_id' => 1524,'is_publish' => 1,'hardware' =>'pc'))->order($orderStr)->select();
		$link2 = M('Link')->where(array('category_id' => 1525,'is_publish' => 1,'hardware' =>'pc'))->order($orderStr)->select();
		$link3 = M('Link')->where(array('category_id' => 1526,'is_publish' => 1,'hardware' =>'pc'))->order($orderStr)->select();
		$link4 = M('Link')->where(array('category_id' => 1527,'is_publish' => 1,'hardware' =>'pc'))->order($orderStr)->select();
		$link5 = M('Link')->where(array('category_id' => 1528,'is_publish' => 1,'hardware' =>'pc'))->order($orderStr)->select();
		$link6 = M('Link')->where(array('category_id' => 1529,'is_publish' => 1,'hardware' =>'pc'))->order($orderStr)->select();
		$link7 = M('Link')->where(array('category_id' => 1530,'is_publish' => 1,'hardware' =>'pc'))->order($orderStr)->select();
		...
		$this->assign('link5',$link5);
		$this->assign('link6',$link6);
		$this->assign('link7',$link7);
		
		// system
		M('System')->where(array('id' => 1))->setinc('person');
		$this->assignSystem($systemDao);
		
		// right广告
		$rightAds = M('Advert')->where(array('category_id' => 1540,'is_publish' => 1))->order('ordernum desc,id desc')->limit(4)->select();
		$this->assign('rightAds',$rightAds);
		
		
		//精选
		$siftList = M('News')->where(array('is_publish' => 1,'is_top' => 1))->order('is_mom desc')->select();
		if(count($siftList) > 5){
			$num = 6;
		}else{
			$num = count($siftList);
		}
		$arr = array_rand($siftList,$num);
		foreach($arr as $k => $v){
			$aa[] = $siftList[$v];
		}
		
		$this->assign('siftList',$aa);
		//}
	}
