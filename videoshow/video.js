...
function player(furl,width,height,autostart,startpic,baseurl){
	...
	if(filetypepaplayer.indexOf(cfiletype)!=-1)//paplayer
	  {
            document.write('<script type="text/javascript" src="/videoshow/swfobject.js"></script>');
...
function ShowPaplayer(furl,width,height,autostart,startpic,baseurl)
 {
    ...
    str+='var so = new SWFObject("/videoshow/paplayer.swf","ply","'+width+'","'+height+'","9","#000000");so.addParam("allowfullscreen","true");so.addParam("allowscriptaccess","always");so.addParam("wmode","opaque");so.addParam("quality","high");so.addParam("salign","lt");so.addVariable("JcScpFile","/videoshow/set.xml");';
